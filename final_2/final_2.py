...
class ParserCBRF:
    ...
    def get_page(self):
        url = f"http://cbr.ru/currency_base/dynamics/?" \
              f"UniDbQuery.Posted=True&" \
              f"UniDbQuery.so=1&" \
              f"UniDbQuery.mode=1&" \
              f"UniDbQuery.date_req1=&" \
              f"UniDbQuery.From=01.07.1992&" \
    ...
    def currency_base_table(self):
        ...
        for i in range(1, len(self.table), 3):
            currency_name = self.table[i]
            currency_amount = self.table[i+1]
            currency_rate = self.table[i+2]
